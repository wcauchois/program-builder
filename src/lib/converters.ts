...
export const convertInt: Converter<number> = (input, argName) => {
  if (!validInt(input)) {
    throw new FlagParseError(`Please provide a valid integer for '${argName}'. Received: ${input}`);
  }
...
export const convertFloat: Converter<number> = (input, argName) => {
  if (!validFloat(input)) {
    throw new FlagParseError(`Please provide a valid float for '${argName}'. Received: ${input}`);
  }
